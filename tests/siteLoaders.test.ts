...
describe('site loaders', () => {
  beforeEach(() => {
    ...
    createRunRepository.mockReturnValue({ listRecent: listRecentRuns });
    listRecentRuns.mockResolvedValue([{ id: 'run-1', dateKey: '2026-05-09', triggerType: 'manual', status: 'completed', startedAt: '2026-05-09T03:30:00.000Z', selectedCount: 1, poolCount: 1 }]);
  });
  ...
  it('falls back to selected items when content_variants is missing from the cloud schema', async () => {
  ...
  it('keeps published items from older runs when content_variants is missing', async () => {
    listPublishedByChannel.mockRejectedValue({
      code: 'PGRST205',
      message: "Could not find the table 'public.content_variants' in the schema cache",
    });
    listRecentRuns.mockResolvedValue([
      { id: 'run-2', dateKey: '2026-05-10', triggerType: 'manual', status: 'completed', startedAt: '2026-05-10T03:30:00.000Z', selectedCount: 1, poolCount: 1 },
      { id: 'run-1', dateKey: '2026-05-09', triggerType: 'manual', status: 'completed', startedAt: '2026-05-09T03:30:00.000Z', selectedCount: 1, poolCount: 1 },
    ]);
    listCandidatesByRun.mockImplementation(async (runId: string) => [
      {
        id: `${runId}-candidate-1`,
        title: runId === 'run-2' ? 'Beta' : 'Alpha',
        source: 'github',
        summary: runId === 'run-2' ? 'Beta summary' : 'Alpha summary',
        rank: 1,
        selectionState: 'selected',
        tags: ['ops'],
        canonicalUrl: `https://example.com/${runId}`,
      },
    ]);
    listSelectedItemsByRun.mockImplementation(async (runId: string) => [
      {
        id: `${runId}-selected-1`,
        candidateId: `${runId}-candidate-1`,
        slug: runId === 'run-2' ? 'beta' : 'alpha',
        title: runId === 'run-2' ? 'Beta' : 'Alpha',
        status: runId === 'run-1' ? 'published' : 'completed',
      },
    ]);
    listArtifactsByRun.mockImplementation(async (runId: string) => [
      {
        selectedItemId: `${runId}-selected-1`,
        artifactType: 'selected_png',
        publicUrl: `https://cdn.example.com/${runId}.png`,
        storagePath: `runs/${runId}/selected/item.png`,
      },
      {
        selectedItemId: `${runId}-selected-1`,
        artifactType: 'selected_markdown',
        publicUrl: `https://cdn.example.com/${runId}.md`,
        storagePath: `runs/${runId}/selected/item.md`,
      },
      {
        selectedItemId: `${runId}-selected-1`,
        artifactType: 'selected_html',
        publicUrl: `https://cdn.example.com/${runId}.html`,
        storagePath: `runs/${runId}/selected/item.html`,
      },
    ]);
    ...
    expect(result.items).toEqual([
      {
        ...
        title: 'Alpha',
        summary: 'Alpha summary',
        coverImageUrl: 'https://cdn.example.com/run-1.png',
        articleUrl: '/site/alpha',
        markdownUrl: 'https://cdn.example.com/run-1.md',
        canonicalSourceUrl: 'https://example.com/run-1',
        publishedAt: '2026-05-09T03:30:00.000Z',
        bodyHtmlStoragePath: 'runs/run-1/selected/item.html',
      },
    ]);
  });
